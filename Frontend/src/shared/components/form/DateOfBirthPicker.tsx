...
const DateOfBirthPicker = (props: { width: string; value?: string; onChange: (date: string) => void }) => {
  const current_year = new Date().getFullYear();
  const years = Array.from({ length: 150 }, (_, i) => current_year - i);
  const months = Array.from({ length: 12 }, (_, i) => i + 1);
  const [daysInMonth, setDaysInMonth] = useState<number[]>([]);
  ...
  // Gửi ngày sinh ra component cha
  useEffect(() => {
  ...
  return (
    <div className={`${props.width} flex gap-[var(--small-gap)]`}>
      <SelectButton
        width="w-1/3"
        value={day}
        onChange={(e: { target: { value: any; }; }) => setDay(Number(e.target.value))}
        dataset={daysInMonth}
      />
      <SelectButton
        width="w-1/3"
        value={month}
        onChange={(e: { target: { value: any; }; }) => setMonth(Number(e.target.value))}
        dataset={months}
      />
      <SelectButton
        width="w-1/3"
        value={year}
        onChange={(e: { target: { value: any; }; }) => setYear(Number(e.target.value))}
        dataset={years}
      />
    </div>
  );
